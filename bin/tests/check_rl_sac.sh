...
if [[ -f /mount/db.dump.pointenv.190821.pkl ]]; then
  ...
  echo "download data"
  bash bin/scripts/download-gdrive 1SllkKVC65W0j3D9G7OdCjgH_kd-4jphd ./data/db.dump.pointenv.190821.pkl
fi

echo "load data to Redis Database"
OMP_NUM_THREADS="1" MKL_NUM_THREADS="1" \
    PYTHONPATH=./examples:./catalyst:${PYTHONPATH} \
    python catalyst_rl/rl/scripts/load_db.py \
    --db="redis" \
    --port=$PORT \
...
OMP_NUM_THREADS="1" MKL_NUM_THREADS="1" \
    PYTHONPATH=./examples:./catalyst:${PYTHONPATH} \
    python catalyst_rl/rl/scripts/run_trainer.py \
...
OMP_NUM_THREADS="1" MKL_NUM_THREADS="1" \
    PYTHONPATH=./examples:./catalyst:${PYTHONPATH} \
    python catalyst_rl/rl/scripts/run_samplers.py \
    --config=./examples/_tests_rl_gym/config_sac_base.yml \
    --db/port="$PORT":str \
    --logdir=./examples/logs/_tests_rl_gym_sac_base &
OMP_NUM_THREADS="1" MKL_NUM_THREADS="1" \
...
OMP_NUM_THREADS="1" MKL_NUM_THREADS="1" \
    PYTHONPATH=./examples:./catalyst:${PYTHONPATH} \
    python catalyst_rl/rl/scripts/dump_db.py \
    --db="redis" \
    --port=$PORT \
...
import pathlib
import numpy as np
from catalyst_rl_rl import utils
reward_goal = -8.0
